...
起動するとタスクトレイに常駐し、クリップボードを監視する。スニップなどで新しい画像が入ると、自動で余白を均等化して書き戻し、整形に成功するたびトースト通知を出す。トレイアイコンのメニューから、監視の有効・無効（**Enabled**）、通知の有無（**Notifications**）を切り替えられ、**Quit** で終了する。
...
| `notify` | `true` | 整形に成功するたびトースト通知を出す |
